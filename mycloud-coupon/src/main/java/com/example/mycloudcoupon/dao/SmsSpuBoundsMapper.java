 ...
 * @author Orall
 * @version 1.0
 * @description SmsSpuBounds的Mapper类
 * @date 2022-10-17
 */
...
public interface SmsSpuBoundsMapper {
    /**
     * @return 以列表形式返回SmsSpuBounds实体类
     * @description 查询所有数据
     * @author Orall
     * @data 2022-10-17
     */
    @Select("select * from sms_spu_bounds")
    List<SmsSpuBounds> listSmsSpuBounds();

    /**
     * @param id 主键id
     * @return 返回SmsSpuBounds实体类
     * @description 根据id获取单条数据（备注：这里的*换成对应想要获取的数据）
     * @author Orall
     * @data 2022-10-17
     */
    @Select("select * from sms_spu_bounds where id=#{id}")
     ...
     * @data 2022-10-17
     */
    @Select("select * from sms_spu_bounds limit #{first},#{second};")
    List<SmsSpuBounds> listSmsSpuBoundsByPage(int first, int second);

    /**
     * @param smsSpuBounds 插入的实体类
     * @return 新增数据的ID
     * @description 插入数据
     * @author Orall
     * @data 2022-10-17
     */
    @Insert("insert into sms_spu_bounds(spu_id,grow_bounds,buy_bounds,work) values(#{spu_id},#{grow_bounds},#{buy_bounds},#{work})")
     ...
     * @data 2022-10-17
     */
    @Update("update sms_spu_bounds set spu_id = spu_id where id=#{id}")
     ...
     * @since 2022-10-17
     */
    @Delete("delete from sms_spu_bounds where id=#{id}")
