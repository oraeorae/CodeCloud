 ...
 * @author Orall
 * @version 1.0
 * @description SmsHomeSubjectSpu的Mapper类
 * @date 2022-10-17
 */
...
public interface SmsHomeSubjectSpuMapper {
    /**
     * @return 以列表形式返回SmsHomeSubjectSpu实体类
     * @description 查询所有数据
     * @author Orall
     * @data 2022-10-17
     */
    @Select("select * from sms_home_subject_spu")
    List<SmsHomeSubjectSpu> listSmsHomeSubjectSpu();

    /**
     * @param id 主键id
     * @return 返回SmsHomeSubjectSpu实体类
     * @description 根据id获取单条数据（备注：这里的*换成对应想要获取的数据）
     * @author Orall
     * @data 2022-10-17
     */
    @Select("select * from sms_home_subject_spu where id=#{id}")
     ...
     * @data 2022-10-17
     */
    @Select("select * from sms_home_subject_spu limit #{first},#{second};")
    List<SmsHomeSubjectSpu> listSmsHomeSubjectSpuByPage(int first, int second);

    /**
     * @param smsHomeSubjectSpu 插入的实体类
     * @return 新增数据的ID
     * @description 插入数据
     * @author Orall
     * @data 2022-10-17
     */
    @Insert("insert into sms_home_subject_spu(name,subject_id,spu_id,sort) values(#{name},#{subject_id},#{spu_id},#{sort})")
     ...
     * @data 2022-10-17
     */
    @Update("update sms_home_subject_spu set name = name where id=#{id}")
     ...
     * @since 2022-10-17
     */
    @Delete("delete from sms_home_subject_spu where id=#{id}")
